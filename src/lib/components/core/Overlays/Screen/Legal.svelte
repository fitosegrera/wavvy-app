...
{#if termsAndConditions}
	<FlexBox
		...
		py="large"
		gap="small"
		class="fixed mobile-width mx-auto top-0 left-[50%] -translate-x-[50%] z-[200] overflow-y-scroll w-full h-full bg-surface">
		...
		{#each Object.keys(privacyPolicy?.Politica_de_Privacidad) as politica}
			<Text intent="p1" class="font-medium capitalize">{politica.split('_').join(' ')}</Text>
				...
				<FlexBox intent="flexColLeft" gap="xsmall">
					{#each Object.keys(privacyPolicy?.Politica_de_Privacidad[politica]) as subPolitica}
						<Text intent="p2" class="font-medium capitalize">
							...
							<span class="font-normal text-[12px] text-on-surface-dark">
		...
		<FlexBox intent="flexRowBetween" gap="small" class="w-full">
			...
			<Button intent="primary" fullWidth onClick={$legalScreenStore.onConfirm}>Acepto</Button>
		</FlexBox>
		<!-- </CollapsibleSection> -->
	</FlexBox>
{/if}
